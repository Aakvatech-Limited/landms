...
class TestPlotApplicationCancellation(FrappeTestCase):
	def test_cancel_linked_draft_sales_order_clears_link_before_delete(self):
		...
		with patch(
			...
			"landms.landms.doctype.plot_application.plot_application.frappe.db.set_value"
		) as set_value, patch(
			"landms.landms.doctype.plot_application.plot_application.frappe.delete_doc"
		) as delete_doc:
		...
		delete_doc.assert_called_once_with(
			"Sales Order", "SAL-ORD-TEST-0001", ignore_permissions=True
		)
		self.assertEqual(doc.sales_order, "")
