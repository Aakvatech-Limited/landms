frappe.ui.form.on("TCB Reconciliation Log", {
  ...
  refresh(frm) {
    ...
    if (
      !frm.is_new() &&
      (status === "Draft" || status === "Failed" || status === "Stopped")
    ) {
      frm
        .add_custom_button(__("Run Reconciliation"), () => {
          frappe.confirm(
            __("Run TCB reconciliation for {0} to {1}?", [
              frm.doc.date_range_start,
              frm.doc.date_range_end,
            ]),
            () => {
              frm.call({
                method: "run",
    ...
    if (!frm.is_new() && status === "Running" && !frm.doc.stop_requested) {
      frm
        .add_custom_button(__("Stop Reconciliation"), () => {
          frappe.confirm(
            __(
              "Request stop for this reconciliation run? It will stop after the current step finishes."
            ),
            () => {
              frm.call({
                method: "request_stop",
...
function _render_status_indicator(frm) {
  ...
  setTimeout(() => {
    ...
    grid.wrapper.find(".grid-row").each(function () {
      ...
      $(this)
        .find('[data-fieldname="row_status"]')
        .css({ color: cfg.color, "font-weight": "700" });
    });
  }, 300);
}
